...
public class SpotReportsActions {
  ...
  public List<Object> viewTodaysSpots() throws Exception {
    ...
    try {
      ...
      String query =
        ...
        "left join users_table on Spot_table.user_id=users_table.user_id" +
        " left join locations on Spot_table.location_id = locations.location_id" +
        " left join spot_category on Spot_table.category_id = spot_category.category_id where Spot_table.registration_date = " +
        "'" +
        currentDate +
        "'";
  ...
  public List<Object> viewTodaysTrendingSpots() throws Exception {
    ...
    try {
      ...
      String query =
        ...
        "'" +
        currentDate +
        "' and Spot_table.status=1 and Spot_table.views >20 limit 1";
  ...
  public List<Object> getReportForAnotherDay(String anotherDate)
    throws Exception {
  ...
  public static void getReportForAnotherMonth(int month, int year)
    throws Exception {
    try {
      CloudStorageConnectionHandler cloudStorageConnection = new CloudStorageConnectionHandler();
      Connection connection = cloudStorageConnection.getConnection();
      ...
      String query =
        ...
        " left join locations on Spot_table.location_id = locations.locationId" +
        " left join spot_category on Spot_table.category_id = spot_category.category_id " +
        "where MONTH(Spot_table.registration_date)= " +
        month +
        " and YEAR(Spot_table.registration_date) =" +
        year;
      //            System.out.println(query);
      ...
      while (iterator.hasNext()) {
        ...
        System.out.println(
          ...
          spot.getUser_name() +
          "\t\t\t\t " +
          spot.getSpot_description() +
          "\t\t\t\t\t " +
          spot.getLocation_name() +
          "\t\t\t\t\t " +
          spot.getCategory_name() +
          "\t\t\t\t\t\t\t\t " +
          spot.getStatus() +
          "\t\t\t\t\t\t\t " +
          spot.getViews() +
          "\t\t\t\t\t\t " +
          spot.getRegistration_date()
        );
      }
      connection.close();
    } catch (Exception e) {
