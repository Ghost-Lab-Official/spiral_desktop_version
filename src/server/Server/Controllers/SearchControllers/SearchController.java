...
public class SearchController {
  ...
  public List<Object> mainMethod(RequestBody requestBody) throws Exception {
    ...
    switch (action) {
      case "getSpots":
        List<Spot> spotsList = new SearchActions()
        .getSpots((Spot) requestBody.getObject());
        for (Spot spot : spotsList) {
          results.add((Object) spot);
        }
        return results;
      case "getPeople":
        List<User> peopleList = new SearchActions()
        .getPeople((User) requestBody.getObject());
        for (User user : peopleList) {
          results.add((Object) user);
        }
        return results;
      case "getMessages":
        List<Comment> messagesList = new SearchActions()
        .getMessages((String) requestBody.getObject());
        for (Comment message : messagesList) {
          results.add((Object) message);
        }
        return results;
      case "searchByPopularity":
        List<Object> popularSpots = new SearchActions()
      ...
      case "RemoveRecentSearch":
        ...
        ResponseStatus responseStatus = new SearchActions()
        .RemoveRecentSearch(user, (RecentSearch) requestBody.getObject());
