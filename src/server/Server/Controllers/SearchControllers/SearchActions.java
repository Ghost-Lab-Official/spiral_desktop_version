...
public class SearchActions {
  ...
  public List<Spot> getSpots(Spot spot) throws Exception {
    ...
    try {
      ...
      String sql =
        "SELECT * from Spot_table WHERE spot_name LIKE '%" +
        searchKey +
        "%' OR spot_description LIKE '%" +
        searchKey +
        "%' AND status = 1 ORDER BY viewers DESC LIMIT 10";
      ...
      if (userId > 0) {
        String insertsql =
          "INSERT INTO searchHistory (searched_query,user_id) values (?,?)";
        PreparedStatement insertHistorystmt = connection.prepareStatement(
          insertsql
        );
  ...
  public List<User> getPeople(User user) throws Exception {
    ...
    try {
      ...
      System.out.println(searchKey);
      String sql =
        "SELECT * from users_table WHERE user_name LIKE '%" +
        searchKey +
        "%' OR first_name LIKE '%" +
        searchKey +
        "%' OR last_name LIKE '%" +
   ...
   **/
  public ResponseStatus RemoveRecentSearch(
    User user,
    ...
    try {
      ...
      PreparedStatement statement = con.prepareStatement(
        "DELETE FROM searchHistory WHERE history_id=? AND user_id=? "
      );
  ...
  public List<Object> getMostPopularSearches() throws Exception {
    ...
    try {
      String SelectRatesquery =
        "select *from Spot_table order by rates desc limit 5";
      String SelectViewsquery =
        "select *from Spot_table order by views desc limit 5";

      String SelectMostSearchedQuery =
        "SELECT searched_query FROM searchHistory GROUP BY searched_query ORDER BY COUNT(searched_query) DESC LIMIT 5";
      ...
      PreparedStatement sq = connection.prepareStatement(
        SelectMostSearchedQuery
      );
